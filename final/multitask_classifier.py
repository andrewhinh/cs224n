...
EDIT:
Added Dora, run with the following:
uv run multitask_classifier.py --fine-tune-mode full-model --batch_size 256 --lr 3e-3 --hidden_dropout_prob 0.01 --rank 8 --use_gpu

Results:
Sentiment classification accuracy: 0.262
Paraphrase detection accuracy: 0.368
Semantic Textual Similarity correlation: -0.010
dev sentiment acc :: 0.262
dev paraphrase acc :: 0.368
dev sts corr :: -0.010

Notes:
bs=256 is the largest that can fit on 24GB GPU
increasing rank led to higher initial train loss
"""
...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.sent = torch.nn.Linear(config.hidden_size, 5)
        self.para = torch.nn.Linear(config.hidden_size, 1)
        self.sim = torch.nn.Linear(config.hidden_size, 1)

        self.rank = config.rank
        self.apply(self._dora)

    def _dora(self, module):
        if hasattr(module, "weight"):
            param_mat = module.weight.data
            param_mat_flat = param_mat.view(param_mat.size(0), -1)
            D, K = param_mat_flat.size()
            V = param_mat_flat.clone()
            mag = torch.norm(V, dim=0, keepdim=True)
            m = torch.empty_like(mag).uniform_(-1, 1)
            weight = m * V / mag
            B = torch.empty((D, self.rank)).uniform_(-1, 1)
            A = torch.empty((self.rank, K)).uniform_(-1, 1)
            module.weight.data = (weight + B @ A).view_as(param_mat)
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        return self.sent(self.dropout(out))
    ...
    def predict_paraphrase(
        ...
        during evaluation.
        """
        out = self.forward(input_ids_1, attention_mask_1)
        out2 = self.forward(input_ids_2, attention_mask_2)
        return self.para(self.dropout(out) - self.dropout(out2))
    ...
    def predict_similarity(
        ...
        out = self.forward(input_ids_1, attention_mask_1)
        out2 = self.forward(input_ids_2, attention_mask_2)
        return self.sim(self.dropout(out) - self.dropout(out2))
