...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        S = S @ value
        B, _, T, _ = S.shape
        S = S.transpose(1, 2).contiguous().view(B, T, self.all_head_size)
        return S
...
class BertLayer(nn.Module):
    ...
    def add_norm(self, input, output, dense_layer, dropout, ln_layer):
        ...
        output_dropped = dropout(output_transformed)
        residual_output = input + output_dropped
        return ln_layer(residual_output)
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        """
        ### TODO
        sa_out = self.self_attention(hidden_states, attention_mask)
        an_1 = self.add_norm(
            hidden_states,
            sa_out,
        ...
        ffw = self.interm_af(self.interm_dense(an_1))
        return self.add_norm(
            hidden_states, ffw, self.out_dense, self.out_dropout, self.out_layer_norm
        )
