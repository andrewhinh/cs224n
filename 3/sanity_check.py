...
def generate_outputs(model, source, target, vocab):
    ...
    torch.save(enc_hiddens, PREFIX_PATH / "sanity_check_en_es_data/enc_hiddens.pkl")
    torch.save(
        dec_init_state, PREFIX_PATH / "sanity_check_en_es_data/dec_init_state.pkl"
    )
    torch.save(enc_masks, PREFIX_PATH / "sanity_check_en_es_data/enc_masks.pkl")
    torch.save(
        combined_outputs, PREFIX_PATH / "sanity_check_en_es_data/combined_outputs.pkl"
    )
    torch.save(target_padded, PREFIX_PATH / "sanity_check_en_es_data/target_padded.pkl")

    # 1f
    # Inputs
    Ybar_t = torch.load(PREFIX_PATH / "sanity_check_en_es_data/Ybar_t.pkl")
    enc_hiddens_proj = torch.load(
        PREFIX_PATH / "sanity_check_en_es_data/enc_hiddens_proj.pkl"
    )
    reinitialize_layers(model)
    # Run Tests
    with torch.no_grad():
    ...
    torch.save(dec_state_target, PREFIX_PATH / "sanity_check_en_es_data/dec_state.pkl")
    torch.save(o_t_target, PREFIX_PATH / "sanity_check_en_es_data/o_t.pkl")
    torch.save(e_t_target, PREFIX_PATH / "sanity_check_en_es_data/e_t.pkl")
...
def question_1d_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    enc_hiddens_target = torch.load(
        PREFIX_PATH / "sanity_check_en_es_data/enc_hiddens.pkl"
    )
    dec_init_state_target = torch.load(
        PREFIX_PATH / "sanity_check_en_es_data/dec_init_state.pkl"
    )
...
def question_1e_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    enc_hiddens = torch.load(PREFIX_PATH / "sanity_check_en_es_data/enc_hiddens.pkl")
    enc_masks = torch.load(PREFIX_PATH / "sanity_check_en_es_data/enc_masks.pkl")
    target_padded = torch.load(
        PREFIX_PATH / "sanity_check_en_es_data/target_padded.pkl"
    )

    # Load Outputs
    combined_outputs_target = torch.load(
        PREFIX_PATH / "sanity_check_en_es_data/combined_outputs.pkl"
    )
    ...
    def stepFunction(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks):
        ...
        o_t = torch.load(
            PREFIX_PATH / "sanity_check_en_es_data/step_o_t_{}.pkl".format(COUNTER[0])
        )
        COUNTER[0] += 1
...
def question_1f_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    # Output
    dec_state_target = torch.load(PREFIX_PATH / "sanity_check_en_es_data/dec_state.pkl")
    o_t_target = torch.load(PREFIX_PATH / "sanity_check_en_es_data/o_t.pkl")
    e_t_target = torch.load(PREFIX_PATH / "sanity_check_en_es_data/e_t.pkl")
...
def main():
    ...
    train_data_src = autograder_read_corpus(
        PREFIX_PATH / "sanity_check_en_es_data/train_sanity_check.es", "src"
    )
    train_data_tgt = autograder_read_corpus(
        PREFIX_PATH / "sanity_check_en_es_data/train_sanity_check.en", "tgt"
    )
    ...
    for src_sents, tgt_sents in batch_iter(
        ...
        tgt_sents = tgt_sents
        break
    vocab = Vocab.load(PREFIX_PATH / "sanity_check_en_es_data/vocab_sanity_check.json")
