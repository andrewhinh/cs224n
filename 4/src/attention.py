...
def precompute_rotary_emb(dim, max_positions):
    ...
    for i in range(1, dim // 2 + 1):
        ...
        for t in range(max_positions):
            rope_cache[t, i - 1, 0] = math.cos(t * theta_i)
            rope_cache[t, i - 1, 1] = math.sin(t * theta_i)
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    _, T, C = x.size()
    for i in range(C // 2):
        cos_values = rope_cache[:T, i, 0]
        sin_values = rope_cache[:T, i, 1]
        rotated_x[:, :, 2 * i] = (
            x[:, :, 2 * i] * cos_values - x[:, :, 2 * i + 1] * sin_values
        )
        rotated_x[:, :, 2 * i + 1] = (
            x[:, :, 2 * i] * sin_values + x[:, :, 2 * i + 1] * cos_values
        )
    ### END YOUR CODE ###
    return rotated_x
