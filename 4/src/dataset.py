...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        noise_len = random.randint(trunc_len // 4 - 1, trunc_len // 4 + 1)
        prefix_start = random.randint(0, trunc_len - noise_len)
        suffix_start = prefix_start + noise_len
        prefix, masked_content, suffix = (
            document[:prefix_start],
            document[prefix_start:suffix_start],
            document[suffix_start:],
        )
        masked_string = (
            prefix
            + self.MASK_CHAR
